...
CONTEXT_SETTINGS = dict(
    default_map={"ip": "10.231.231.128", "name": "TDSXX4A", "address": 4, "timeout": 3}
)
...
def cli(ctx, ip, name, address, timeout, debug):
    ...
    ctx.obj = dict(ip=ip, name=name, address=address, timeout=timeout)
    pass


@click.option(
    "--sources", type=str, multiple=True, default=["CH1"], help="Input channel"
)
...
@click.option("--arg", default="curve?", help="Sleep time at aquisition")
@cli.command()
@click.pass_context
def take_data(ctx, sources, record_length, hscale, sleep_ms, arg):
...
@cli.command()
@click.pass_context
def take_data_11801(ctx, source, record_length, hscale, sleep_ms):
    device = TDSXX4ADevice(ctx.obj["address"], ctx.obj["ip"], ctx.obj["timeout"])
    device.connect()
    device.setup()
...
@click.option(
    ...
    type=str,
    default="-100",
    help="Suppress values less than this (dB)",
)
@cli.command()
@click.pass_context
